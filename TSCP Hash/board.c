...
int get_hash() 
{
	...
	for (i = 0; i < 64; ++i)
		if (color[i] != EMPTY)
			temp_hash ^= hash_piece[color[i]][piece[i]][i];
	if (side == DARK)
		temp_hash ^= hash_side;
	if (ep != -1)
		temp_hash ^= hash_ep[ep];
	return temp_hash;
}
...
BOOL makemove(move_bytes m)
{
	...
	/* move the piece */
	assert(color[(int)m.from] != EMPTY);
	assert(piece[(int)m.from] != EMPTY);
	assert((int)m.to >= 0 && (int)m.to < 64);
	assert((int)m.from >= 0 && (int)m.from < 64);
